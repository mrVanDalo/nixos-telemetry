...
in
{
  ...
  config = lib.mkMerge [
    ...
    # opentelemetry wiring: the influxdb receiver — fires when the host has
    # shared-network containers (their telegraf pushes here over loopback),
    # even if telegraf is not enabled on the host itself
    # ---------------------------------------------------------------
    (mkIf
      (
    ...
    # telegraf agent config — only when telegraf is enabled
    # -------------------------------------------------------
    (lib.mkIf (config.telemetry.enable && config.telemetry.telegraf.enable) (
      lib.mkMerge [
        ...
        (mkIf (config.telemetry.telegraf.inputs.procstat.pattern != null) {
          services.telegraf.extraConfig.inputs.procstat.pattern =
            config.telemetry.telegraf.inputs.procstat.pattern;
        })
        ...
        (mkIf
          (
            config.telemetry.telegraf.autowire.nginx.enable
            && config.services.nginx.enable
            && config.services.nginx.statusPage
          )
          {
        ...
        # -----------------------------
        (mkIf (config.telemetry.telegraf.autowire.memcached.enable && config.services.memcached.enable)
          (mkMerge [
        ...
        # ---------------------------------
        (mkIf
          (config.telemetry.telegraf.autowire.elasticsearch.enable && config.services.elasticsearch.enable)
          {
        ...
        (mkIf
          (
            config.telemetry.telegraf.autowire.rabbitmq.enable
            && config.services.rabbitmq.managementPlugin.enable
          )
          {
